...
Clique em Criar pilha → Com novos recursos (padrão). alt text
...
Faça o upload do arquivo EC2.yaml (ou EC2.json). alt text

Revisar Infraestrutura
Após o upload, você pode:

Visualizar graficamente no Designer do CloudFormation. alt text alt text

Editar manualmente arrastando os componentes.

Alterar diretamente no código na aba Modelo. alt text

Configurar a Pilha
Dê um nome para a pilha (exemplo: Ecommerce-Stack). alt text

Selecione os parâmetros desejados (como tipo da instância EC2).

Clique em Avançar até a tela de revisão.

Confirme e clique em Criar pilha. alt text alt text
